...
export default function CategoryProducts() {
    ...
    let { data, isLoading, isError, error, isFetching } = useQuery(
        {
            queryKey: ['prodcutCategries'],
            queryFn: getAllCategories,
            select: (data) => data?.data.data.filter((product) => product.category._id == id),
        }
    )

    if (isLoading) {
        return <div className='flex justify-center items-center w-full h-svh'><i className='fas fa-spin fa-spinner text-5xl'></i></div>
    }


    return (<>
        <div className='flex flex-wrap gap-y-10 xl:p-10'>

            <div className='flex flex-wrap p-5'>
                {data?.map((product) => {
                    return <div key={product.id} className="product w-full md:w-1/2 xl:w-1/4 p-3">
                        <div className="relative flex flex-col items-center justify-center ">
                            <div className="container">
                                <div className="w-full bg-slate-400 shadow-lg rounded-xl p-2">
                                        ...
                                        <div className="flex-auto justify-evenly">
                                            <div className="flex flex-wrap ">
                                                <div className="w-full flex-none text-sm flex items-center text-gray-600">
                                                    ...
                                                    <span className="text-gray-100 whitespace-nowrap mr-3">{product.ratingsAverage}</span><span className="mr-2 text-gray-100">{product.category.name}</span>
                                                </div>
                                                <div className="flex items-center w-full justify-between min-w-0 ">
                                                    <h2 className="text-lg mr-auto cursor-pointer text-gray-200 hover:text-purple-500 truncate">{product.title.split(' ').slice(0, 4).join(' ')}</h2>
                                            ...
                                            <div className="flex space-x-2 text-sm font-medium justify-start mt-5">
                                                <button onClick={() => addProductToCart(product.id)} className="transition-all w-full ease-in duration-300 inline-flex items-center text-sm font-medium mb-2 md:mb-0 bg-purple-500 px-5 py-2 hover:shadow-lg hover:tracking-widest text-white rounded-full hover:bg-purple-600 ">
                                                    <span>Add Cart</span>
                                                </button>
